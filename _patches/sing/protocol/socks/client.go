...
func (c *Client) DialContext(ctx context.Context, network string, address M.Socksaddr) (result net.Conn, err error) {
	...
	case Version5:
		...
		bindAddr := response.Bind
		// Some SOCKS servers advertise 0.0.0.0/::/loopback as UDP relay;
		// dial the TCP peer IP with the advertised port instead.
		if !bindAddr.IsValid() || bindAddr.Addr.IsUnspecified() || bindAddr.Addr.IsLoopback() {
			if tcpAddr, ok := tcpConn.RemoteAddr().(*net.TCPAddr); ok {
				bindAddr = M.SocksaddrFrom(M.AddrFromIP(tcpAddr.IP), bindAddr.Port)
			}
		}
		var udpConn net.Conn
		udpConn, err = c.dialer.DialContext(ctx, N.NetworkUDP, bindAddr)
		if err != nil {
			tcpConn.Close()
			return nil, err
		}
